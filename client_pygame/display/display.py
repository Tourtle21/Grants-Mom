...
class Display(BaseDisplay):
    ...
    def paint_pregame(self, surface, control):
        ...
        file_path = os.path.join('display', 'images', 'title.png')
        ...
        (mX, mY) = position
        self.draw_text_center(surface, "X & Y: "+str(mX)+" "+str(mY), self.text_color, 0, 100, self.font)
    ...
    def paint_waiting_for_game(self, surface, engine, control):
        ...
        rect = pygame.Rect(0, 0, self.width, self.height)
        rect = pygame.Rect(0, 0, self.width, self.height)
        file_path = os.path.join('display', 'images', 'backdrop.png')
        image = pygame.image.load(file_path)
        surface.blit(image, rect)
        # text message in center of screen
    ...
    def paint_wall(self, surface, engine, control, obj):
        ...
        file_path = os.path.join('display', 'images', 'asteroid.gif')
        image = pygame.image.load(file_path)
        surface.blit(image, rect)
        return
    ...
    def paint_npc(self, surface, engine, control, obj):
        ...
        Draws living NPCs.
        """
        health_dots = int(round((obj.get_health() / obj.get_max_health()) * 6))
        health_bar = ''
        health_bar = health_bar.ljust(health_dots, '*')
        self.draw_text_center(surface, health_bar, self.text_color, obj.get_x(), obj.get_y(), self.font)
        rect = self.obj_to_rect(obj)
        if obj.is_alive():
            file_path = os.path.join('display', 'images', 'npc.png')
        else:
            file_path = os.path.join('display', 'images', 'boom.png')
    ...
    def paint_missile(self, surface, engine, control, obj):
        ...
        if obj.is_alive():
            if hasattr(control, 'missile_type'):
                kind = control.missile_type
            else:
                kind = "bomb.png"
            file_path = os.path.join('display', 'images', kind)
            image = pygame.image.load(file_path)
            surface.blit(image, rect)
        return
    ...
    def paint_player(self, surface, engine, control, obj):
        ...
        """
        health_dots = int(round((obj.get_health() / obj.get_max_health()) * 10))
        health_bar = ''
        health_bar = health_bar.ljust(health_dots, '*')
        self.draw_text_center(surface, health_bar, self.text_color, obj.get_x(), obj.get_y(), self.font)
        ...
        if obj.is_alive():
            ...
            if obj.get_oid() == engine.get_player_oid():
                file_path = os.path.join('display', 'images', 'saucer2.png')

            else:
                file_path = os.path.join('display', 'images', 'Untitled-3.png')
        else:
            file_path = os.path.join('display', 'images', 'boom.png')
        rect = self.obj_to_rect(obj)
        image = pygame.image.load(file_path)
